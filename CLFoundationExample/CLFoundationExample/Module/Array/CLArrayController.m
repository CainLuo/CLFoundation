...
- (void)cl_array {
    ...
    [self.cl_textViewString appendFormat:@"NSArray:\n创建数组: %@\n", cl_safeArray];
    [self.cl_textViewString appendFormat:@"获取对象的索引: %lud\n", (unsigned long)[cl_safeArray cl_safeIndexOfObject:@"Hello Word"]];
    [self.cl_textViewString appendFormat:@"获取指定区间的数组: %@\n", [cl_safeArray cl_safeSubarrayWithRange:NSMakeRange(0, 10)]];
    [self.cl_textViewString appendFormat:@"获取指定索引的对象: %@\n", [cl_safeArray cl_safeObjectAtIndex:10]];
    [self.cl_textViewString appendFormat:@"获取指定Plist文件的数组: %@\n\n", [NSArray cl_arrayWithPlistName:@"Array"]];
}
...
- (void)cl_mutableArray {
    ...
    [self.cl_textViewString appendFormat:@"NSMutableArray:\n安全的添加一个对象: %@\n", cl_safeMutableArray];
    
    [cl_safeMutableArray cl_insertSafeObject:@"13800138000"
                                       index:10];
    
    [self.cl_textViewString appendFormat:@"安全的插入一个对象: %@\n", cl_safeMutableArray];
    ...
    [self.cl_textViewString appendFormat:@"安全的插入一个数组: %@\n", cl_safeMutableArray];
    
    [cl_safeMutableArray cl_safeRemoveObjectAtIndex:10];
    
    [self.cl_textViewString appendFormat:@"安全的删除索引为10的对象: %@\n", cl_safeMutableArray];
    
    [cl_safeMutableArray cl_safeRemoveObjectsInRange:NSMakeRange(2, 1)];
    
    [self.cl_textViewString appendFormat:@"安全的区域为2~1的对象: %@\n", cl_safeMutableArray];
}
